...
저장소는 PostgreSQL 하나다. S3와 Redis는 곁다리로 둔다. **S3에는 이미지 원본이 있고 Redis에는 사본만 있다.** Redis가 통째로 사라져도 PostgreSQL에서 다시 만들 수 있어야 하고, 죽어 있는 동안에도 기능이 멈추지 않아야 한다 ([자동완성 캐시 운영](../operations/keyword-cache.md)). 캐시를 원본으로 삼는 기능은 두지 않는다.
