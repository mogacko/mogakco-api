"""move profile image reference into media usages
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("usage_type IN ('PROFILE')", name="ck_media_usages_type"),
        sa.UniqueConstraint("usage_type", "usage_id", name="uq_media_usages_slot"),
    )
    op.create_index("ix_media_usages_asset_id", "media_usages", ["asset_id"])

    # 이미 붙어 있는 프로필 사진을 사용처 행으로 옮긴다.
    op.execute(
        """
        INSERT INTO media_usages (asset_id, usage_type, usage_id)
...
def downgrade() -> None:
    ...
    op.execute(
        ...
        UPDATE users SET profile_image_asset_id = (
            SELECT asset_id FROM media_usages
            WHERE media_usages.usage_type = 'PROFILE' AND media_usages.usage_id = users.id
        )
        """
    )

    op.drop_index("ix_media_usages_asset_id", table_name="media_usages")
